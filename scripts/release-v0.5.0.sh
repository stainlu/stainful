#!/usr/bin/env bash
# Post-publish: commit + tag + push + GitHub Release for stainful v0.5.0.
...
# 1. Verify PyPI has v0.5.0 (no point committing the release if it isn't live)
...
if gh release view "$TAG" >/dev/null 2>&1; then
  ...
  gh release create "$TAG" \
    --title "$TAG — TypeScript ships; Python gap list closed" \
