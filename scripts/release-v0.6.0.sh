#!/usr/bin/env bash
# Post-publish: commit + tag + push + GitHub Release for stainful v0.6.0.
...
# 1. Verify PyPI has v0.6.0 (no point committing the release if it isn't live)
...
# 2. Commit the release (pyproject + CHANGELOG + uv.lock + memory + drafts)
if ! git diff --quiet || ! git diff --cached --quiet; then
  echo "==> Committing release: $TAG"
  git add -A
  git commit -m "release: $TAG

See CHANGELOG.md [$VERSION] — TypeScript completeness pass (nested
resources, multipart/binary uploads, webhook unwrap).
...
if gh release view "$TAG" >/dev/null 2>&1; then
  ...
  gh release create "$TAG" \
    --title "$TAG — TypeScript completeness: uploads, webhooks, nested resources" \
