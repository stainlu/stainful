...
# Python builtin type names that, when used as a field name, would *shadow*
# the builtin if a later field annotation referenced it as a type. The fix
# (matches what Stainless does in openai-python's Conversation class): emit
# the field LAST among its required-vs-optional cohort so it doesn't shadow
# earlier annotations. Other builtins (`type`, `list`, `dict`, ...) almost
# never appear as bare annotations — `type` is a soft keyword we use as a
# discriminator field name, and `list`/`dict` are typically `List[...]`/
# `Dict[...]` in our output — so they're omitted.
_SHADOWING_BUILTIN_NAMES = frozenset({"object"})


def _page_class_kind(name: str) -> str:
    """`pagination[].name` from stainless.yml → page-class display name
    `<Pascal>Page`. The page suffix is dedup'd so `cursor_page` →
    `CursorPage` (not `CursorPagePage`) AND bare `page` → `Page`.
    """
    p = pascal(name)
    return p if p.endswith("Page") else p + "Page"


def _reorder_builtin_shadows(lines: list[str]) -> list[str]:
    """Reorder field lines so a field named like a Python builtin (e.g.
    `object`) doesn't shadow that builtin's use as a type in *earlier*
    fields. Real example from the openai spec — `ConversationResource`:

        id: str
        object: Literal["conversation"]
        metadata: object        # mypy: `object` resolves to the field above
        created_at: int

    With this rule the offending field moves to AFTER the last sibling
    that references its name as a type — matching openai-python's actual
    layout (`id`, `created_at`, `metadata`, `object`). Required-vs-optional
    cohorts are preserved (pydantic requires defaulted fields after
    non-defaulted ones); if moving the field would cross the cohort
    boundary, leave it (caller will get a mypy error and we should add a
    different fix).
    """
    if not lines:
        return lines
    # parse: per line, extract (name, is_required). Discriminator/aliased
    # lines look like `    name: T = Field(alias="...")`; treat `Field(`
    # as a default just like any `= …` (the alias doesn't change required
    # semantics, but pydantic ordering treats `Field(default=...)` as
    # defaulted — Stainless emits these in the required cohort too, so we
    # follow suit: a `Field(` with an explicit `default=` IS defaulted,
    # otherwise NOT).
    def _parse(line: str) -> tuple[str | None, bool]:
        m = re.match(r"^    (\w+):\s*(.*)$", line)
        if not m:
            return None, True                       # not a field line
        name, rhs = m.group(1), m.group(2)
        has_default = bool(
            re.search(r"=\s*(?!Field\b)", rhs)       # `= <not-Field>`
            or re.search(r"Field\([^)]*\bdefault\b", rhs)
        )
        return name, not has_default
    parsed = [_parse(line) for line in lines]
    out = list(lines)
    moved_any = False
    # Iterate by name (not by index) since indices shift after each move.
    # One pass is enough: we move each shadowing field once.
    for target_name in _SHADOWING_BUILTIN_NAMES:
        i = next(
            (k for k, (nm, _) in enumerate(parsed) if nm == target_name), None
        )
        if i is None:
            continue
        # find LATER fields whose annotation uses `target_name` as a bare
        # type — those are the ones being shadowed by the class-attribute
        # binding from line i. Mypy resolves the annotation to the local
        # binding (the field), not the builtin → error.
        last_shadowed = -1
        for j in range(i + 1, len(parsed)):
            if parsed[j][0] is None:
                continue
            rhs = out[j].split(":", 1)[1] if ":" in out[j] else ""
            if re.search(rf"\b{re.escape(target_name)}\b", rhs):
                last_shadowed = j
        if last_shadowed < 0:
            continue                                # no forward shadowing
        # preserve required-vs-optional cohort (pydantic requires defaulted
        # fields after non-defaulted ones).
        my_required = parsed[i][1]
        cohort_end = max(
            j for j, (_, r) in enumerate(parsed) if r == my_required
        )
        target = min(last_shadowed, cohort_end)
        if target <= i:
            continue
        # Pop at i, insert at `target`. After popping at i, indices j > i
        # shift down by 1; inserting at `target` (= old j after the shift)
        # places the field immediately AFTER the originally-at-`target`
        # line. That's the position we want.
        out.insert(target, out.pop(i))
        parsed.insert(target, parsed.pop(i))
        moved_any = True
    return out if moved_any else lines
...
class _Emitter:
    def __init__(self, api: API, out_dir: str) -> None:
        ...
        # Same for bi-directional variants → alias to BiDirectionalPage.
        self._bidir_aliases: set[str] = set()
    ...
    def _render(self, t: Type, path: str, seen: frozenset[str]) -> str:
        ...
        if isinstance(t, UnionType):
            # When a discriminator is in play, *every* variant must carry
            # the discriminator field — pydantic refuses to apply a
            # discriminator over an `Any`/`object` variant (real spec
            # example: openai `filters: oneOf: [{type:object,
            # properties:{type, ...}}, {}]` where the empty branch lowers
            # to AnyType). Drop the un-discriminatable variants in that
            # case. If nothing structured remains, fall through to a plain
            # untagged union.
            variants = list(t.variants)
            if t.discriminator is not None:
                discriminable = [v for v in variants if not isinstance(v, AnyType)]
                if discriminable:
                    variants = discriminable
            parts = [
            ...
            if t.discriminator is not None and "object" not in nn:
                return (
                    f'Annotated[{union}, '
    ...
    def _emit_object(
        ...
        header = f"class {cls}({', '.join(bases) or 'BaseModel'}):"
        if not obj.properties:
            body = ["    pass"]
        else:
            body = _reorder_builtin_shadows(
                [self._field(p, cls, seen) for p in obj.properties]
            )
    ...
    def _pagination(self, m: Method, resource: str, *, is_async: bool):
        ...
        # Bi-directional has its own algorithm — distinct class in the
        # runtime; aliased to the user-given page name if set.
        if m.pagination.bidirectional:
            if m.pagination.name:
                kind = _page_class_kind(m.pagination.name)
                self._bidir_aliases.add(kind)
            else:
                kind = "BiDirectionalPage"
        elif m.pagination.name:
            kind = _page_class_kind(m.pagination.name)
        else:
        ...
        # cursor variants share `_CursorPage`'s algorithm; bi-directional
        # variants share `_BiDirectionalPage`).
        if not m.pagination.bidirectional:
    ...
    def _method_src(self, m: Method, resource: str, *, is_async: bool) -> str:
        ...
        if pag:
            ...
            # Bi-directional shapes (anthropic-style) carry distinct before/
            # after params + first/last fields.
            cfg_items: list[str] = []
            if m.pagination and m.pagination.bidirectional:
                if m.pagination.bi_before_param:
                    cfg_items.append(
                        f'"before_param": "{m.pagination.bi_before_param}"'
                    )
                if m.pagination.bi_after_param:
                    cfg_items.append(
                        f'"after_param": "{m.pagination.bi_after_param}"'
                    )
                if m.pagination.bi_first_field:
                    cfg_items.append(
                        f'"first_field": "{m.pagination.bi_first_field}"'
                    )
                if m.pagination.bi_last_field:
                    cfg_items.append(
                        f'"last_field": "{m.pagination.bi_last_field}"'
                    )
            else:
    ...
    @staticmethod
    def _walk(resources: list[Resource], parent: tuple[str, ...] = ()):
        """Yield (resource, parent_path) for every resource. parent_path is
        the chain of snake-case names from the root, *not* including the
        resource itself. Used for nested directory emission (openai-python
        layout: `resources/chat/completions/messages.py`).
        """
        for r in resources:
            yield r, parent
            yield from _Emitter._walk(r.subresources, parent + (snake(r.name),))
    ...
    def _resource_src(self, r: Resource) -> str:
        ...
        models_import = (
            f"from {self.pkg}.types import " + ", ".join(used) + "\n"
            if used else ""
        )
        ...
        webhook_import = (
            f"from {self.pkg}._core._webhooks import (\n"
        ...
        stream_import = (
            f"from {self.pkg}._core._streaming import AsyncStream, Stream\n"
        ...
        pages = sorted({
            ...
            for m in re.finditer(
                r"\b(?:Sync|Async)(?:[A-Z][A-Za-z]*)?Page\b", scan
            )
        })
        pag_import = (
...
from {self.pkg}._core._request_options import make_request_options
from {self.pkg}._core._resource import AsyncAPIResource, SyncAPIResource
from {self.pkg}._core._response import (
    async_to_raw_response_wrapper,
    async_to_streamed_response_wrapper,
...
'''
    ...
    def emit(self) -> None:
        ...
        # 2b. nested directory layout (matches openai-python):
        #       resources/embeddings.py                       (leaf)
        #       resources/chat/__init__.py + chat/chat.py     (has subresources)
        #       resources/chat/completions/__init__.py
        #         + chat/completions/completions.py           (has subresources)
        #       resources/chat/completions/messages.py        (leaf)
        #     Two resources with the same leaf name (e.g. `messages` under
        #     `chat.completions` AND under `threads`) now coexist — they live
        #     in different directories.
        resources_root = self.root / "resources"
        walked = list(self._walk(self.api.root.subresources))
        # Render all resource sources up-front (registers model classes
        # eagerly, so `_emit_models` sees the full set).
        rendered = [(r, parent, self._resource_src(r)) for r, parent in walked]
        self._emit_models()
        # Subpackage __init__ contents: top-level + every intermediate dir
        # re-exports its own resource's classes (so `from <pkg>.resources
        # import ChatResource` resolves whether `chat` is a leaf file or a
        # subpackage).
        for r, parent, src in rendered:
            name = snake(r.name)
            if r.subresources:
                # `resources/<parent>/<name>/<name>.py` + `__init__.py`
                ...
                # leaf — `resources/<parent>/<name>.py`
                d = resources_root.joinpath(*parent)
                d.mkdir(parents=True, exist_ok=True)
                (d / f"{name}.py").write_text(src)
        # Top-level `resources/__init__.py` re-exports only the top-level
        # resource classes — those used directly by `<pkg>/_client.py`.
        resources_root.mkdir(parents=True, exist_ok=True)
        (resources_root / "__init__.py").write_text(
            _HEADER
            + "".join(
        ...
        # imports resolve. Forward-only cursor variants alias to
        # `SyncCursorPage`/`AsyncCursorPage`; anthropic-style bi-
        # directional variants alias to `SyncBiDirectionalPage`/
        # `AsyncBiDirectionalPage`. Runtime built-ins (`CursorPage`,
        # `Page`, `BiDirectionalPage`) need no alias line.
        extra_cursor = sorted(
            k for k in self._page_aliases
            if k not in ("CursorPage", "Page", "BiDirectionalPage")
        )
        extra_bidir = sorted(
            k for k in self._bidir_aliases if k != "BiDirectionalPage"
        )
        if extra_cursor or extra_bidir:
            pag_path = self.root / "_core" / "pagination.py"
            block = ["", "", "# Stainful: spec-specific page-class aliases."]
            for kind in extra_cursor:
                block.append(f"Sync{kind} = SyncCursorPage")
                block.append(f"Async{kind} = AsyncCursorPage")
            for kind in extra_bidir:
                block.append(f"Sync{kind} = SyncBiDirectionalPage")
                block.append(f"Async{kind} = AsyncBiDirectionalPage")
